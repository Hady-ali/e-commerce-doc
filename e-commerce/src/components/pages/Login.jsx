...
// import { Link } from "react-router-dom"; // أضف هذا الاستيراد في الأعلى
// import { useFormik } from "formik";
// import { useState, useEffect, useRef } from "react";
// import * as Yup from "yup";
// import { 
...
//   FaApple
// } from "react-icons/fa";
// import { FacebookAuthProvider, getAuth, GoogleAuthProvider, signInWithEmailAndPassword } from "firebase/auth";
// import { useNavigate } from "react-router-dom";
// import { app } from "../../firebase.config";
// import { useDispatch } from "react-redux";
// import { setUser } from "../../redux/userSlice";
...
const Login = () => {
  ...
  const navigate = useNavigate();
  const dispatch = useDispatch();
  // const { formRef, handleFocus, handleBlur } = useAnimation();
  ...
  const handleSocialLogin = useCallback(async (provider) => {
    try {
      ...
      setErrorFirebase(errorMessages[error.code] || 
  ...
  const renderFormField = (fieldName, config) => {
    const { label, icon, type, placeholder } = config;
    const isError = formik.touched[fieldName] && formik.errors[fieldName];
    
    return (
      <div className={`form-group ${fieldName} ${isError ? "error" : ""}`}>
        ...
        <div className="input-container">
          <input
            ...
            type={type}
            onChange={formik.handleChange}
            // onBlur={(e) => handleBlur(fieldName, e)}
        ...
        {isError && (
          <div className="error-message animate-fade">{formik.errors[fieldName]}</div>
        )}
      </div>
    );
  };
